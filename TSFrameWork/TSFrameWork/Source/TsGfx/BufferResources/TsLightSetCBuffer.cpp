...
{
    m_pLightRefList.reserve(LIGHT_CB_MAX);
}
TsLightSetCBuffer::~TsLightSetCBuffer()
{
...
{
	CreateCBuffer( pDev , &m_CBufferData , TS_CBUFFER_REGISTER::LightSetCB , TS_SHADER_TYPE::VP_SHADER );
...
TsBool TsLightSetCBuffer::UpdateCBuffer(TsDeviceContext* pDevContext)
{
    ...
    //if (isUpdate)
    {
        m_CBufferData.lightNum = m_pLightRefList.size();
        for (TsUint i = 0; i < m_pLightRefList.size(); ++i)
        {
            TsLight* pLight = m_pLightRefList[i];
			m_CBufferData.lightData[i].type = ( TsUint )pLight->GetLightType();
			m_CBufferData.lightData[i].pos = pLight->GetWorldPosition();
			m_CBufferData.lightData[i].dir = TsVector3::zero;
			m_CBufferData.lightData[i].color = pLight->GetColor().m_color;
            if (pLight->GetLightType() == TsLight::LIGHT_TYPE::LIGHT_DIRECTIONAL)
            {
                TsVector3 v = TsVector3::front;
                v *= pLight->GetWorldMatrix();
                v.Normalize();
				m_CBufferData.lightData[i].dir = v;
            }

			m_CBufferData.lightData[i].intensity = pLight->GetIntensity();
            
			m_CBufferData.lightData[i].angle = 0;
            if (pLight->GetLightType() == TsLight::LIGHT_TYPE::LIGHT_SPOT)
            {
                // todo Set Spot Light Angle
            }

			m_CBufferData.lightData[i].range = 0;
            if (pLight->GetLightType() == TsLight::LIGHT_TYPE::LIGHT_SPOT || 
            ...
            if (pLight->GetLightType() == TsLight::LIGHT_TYPE::LIGHT_DIRECTIONAL)
            {
				...
				lispsm.SetLightDir( m_CBufferData.lightData[i].dir );
                lispsm.SetSceneBoundingBox( TsAABB3D( TsVector3( -250 , -10 , -250 ) , TsVector3( 250 , 300 , 250 ) ) );
                lispsm.UpdateShadowMatrix();
				m_CBufferData.lightData[i].worldToShadowMatrix = lispsm.GetLVPMatrix().Transposed();
            }
            else
				m_CBufferData.lightData[i].worldToShadowMatrix = TsMatrix::identity;
        }
		ChangedCBuffer( pDevContext , &m_CBufferData );
    }
    m_isUpdate = TS_FALSE;
    return TS_TRUE;
}
